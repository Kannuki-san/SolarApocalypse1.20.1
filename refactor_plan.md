...
## 2026-07-09 CurseForge description draft

CurseForgeページへ貼る説明文を2.0の現在仕様に合わせて整理した。

対応。

* `curseforge_Description.md` をGit管理外の公開ページ用メモとして更新
* `.gitignore` に `curseforge_Description.md` を追加
* 英語説明、日本語説明、進行、特徴、救済loot、サーバー側導入、注意事項を現在仕様に合わせて記載
...
* `classPaths: []` と `modulePaths: []` による回避はクライアント起動を壊す可能性があるため、ローカル `.vscode/launch.json` から戻した
* VS CodeのJava Debug構成でサーバーが二重読み込みする場合は、`Gradle: runServer` または `./gradlew runServer` を使う
